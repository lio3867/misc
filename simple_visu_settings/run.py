...
def send_settings():
    ...
    Send the settings
    '''
    with open('simple_visu_settings/settings.yaml', 'r') as f_r:
        settings = yaml.load(f_r, Loader=yaml.FullLoader)
        print(f'settings: {settings}')
    str_set = json.dumps(settings)
    emit('settings', str_set)


@socketio.on('retrieved_settings')
def retrieved_settings(mess):
    '''
    Receiving the settings
    '''
    print(f"Receiving the settings !!!")
